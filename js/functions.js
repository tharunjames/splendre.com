...
jQuery(document).ready(function ($) {
	...
	$(".upload-btn input").change(function () {
		var fileName = $(this).val();
		$(this).parent().parent().children('.fake-input').html(fileName);
	});

	$(".apply-now").click(function () {
		var jobName = $(this).parents('.job-wrapper').find('.job-name').text();
		$('#myJob').val(jobName);
	});

	$('form[name="career"]').submit(function (e) {
		//alert('test');
		e.preventDefault();
		var This = $(this);
		...
		if ($(this).valid()) {
			$.ajax({
				...
				success: function (response) {
					if (response == 'F12') {
						$('#response_msg').html('Invalid Captcha');
						$('#response_msg').slideDown('slow');
					} else {
						$('#career-form')[0].reset();
						console.log(response);
						$('#response_msg').html('Thank you for your interest. We will get back to you shortly.');
						$('#response_msg').slideDown('slow');
